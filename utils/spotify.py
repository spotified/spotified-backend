    ...
    if uri_link_or_id.lower().startswith("spotify:"):
        if not uri_link_or_id.lower().startswith("spotify:{}:".format(content_type)):
            raise ValidationError(
                {"spotify_id": _("This is not a Spotify {} URI".format(content_type))}
            )
        ...
        try:
            if not url_pathes_parsed[-2] == content_type:
                raise ValidationError(
                    {
                        "spotify_id": _(
                            "This is not a Spotify {} Link".format(content_type)
                        )
                    }
                )
            ...
            raise ValidationError(
                {"spotify_id": _("This is not a Spotify {} Link".format(content_type))}
            )
    else:
